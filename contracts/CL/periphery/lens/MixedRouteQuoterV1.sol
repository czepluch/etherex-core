...
import {TickBitmap} from "contracts/CL/core/libraries/TickBitmap.sol";
...
import {Path} from "contracts/CL/periphery/libraries/Path.sol";
import {PoolAddress} from "contracts/CL/periphery/libraries/PoolAddress.sol";
import {CallbackValidation} from "contracts/CL/periphery/libraries/CallbackValidation.sol";
...
import {PoolTicksCounter} from "contracts/CL/periphery/libraries/PoolTicksCounter.sol";
import {RamsesLegacyLibrary} from "contracts/CL/universalRouter/modules/uniswap/v2/RamsesLegacyLibrary.sol";
...
contract MixedRouteQuoterV1 is IMixedRouteQuoterV1, IUniswapV3SwapCallback {
    ...
    function getPool(address tokenA, address tokenB, int24 tickSpacing) private view returns (IRamsesV3Pool) {
        return IRamsesV3Pool(
            PoolAddress.computeAddress(contractDeployer, PoolAddress.getPoolKey(tokenA, tokenB, tickSpacing))
        );
    }
        ...
        returns (uint256)
    {
        ...
        (uint256 reserveIn, uint256 reserveOut, uint256 decimalsIn, uint256 decimalsOut) =
    ...
    function handleV3Revert(bytes memory reason, IRamsesV3Pool pool, uint256 gasEstimate)
        private
        view
        ...
        )
    {
        ...
        while (true) {
            ...
            if (tickSpacingUint & flagBitmask != 0) {
                ...
                (amountIn, v3SqrtPriceX96AfterList[i], v3InitializedTicksCrossedList[i], _gasEstimate) =

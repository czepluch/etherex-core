...
library NFTSVG {
    ...
    function generateSVG(string memory defs, string memory body) internal pure returns (string memory svg) {
        ...
        version: "2"
        */
        return string.concat(defs, body, "</svg>");
    }

    function generateSVGBody(SVGBodyParams memory params) internal pure returns (string memory body) {
        /// @dev concat string in steps to avoid stack-too-deep
        body =
            generateSVGBorderText(params.quoteToken, params.baseToken, params.quoteTokenSymbol, params.baseTokenSymbol);
        body =
            string.concat(body, generateSVGCardMantle(params.quoteTokenSymbol, params.baseTokenSymbol, params.feeTier));
        body = string.concat(
            body, generageSvgCurve(params.tickLower, params.tickUpper, params.tickSpacing, params.overRange)
        );
        body = string.concat(
            body, generateSVGPositionDataAndLocationCurve(params.tokenId.toString(), params.tickLower, params.tickUpper)
        );
        body = string.concat(body, generateSVGRareSparkle(params.tokenId, params.poolAddress));
    }
        ...
        returns (string memory svg)
    {
        ...
        /// @dev concat string in steps to avoid stack-too-deep
        svg = string.concat(
            ' <g style="transform:translate(29px, 384px)">', '<rect width="', uint256(7 * (str1length + 4)).toString()
        );
